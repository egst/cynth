...
namespace cynth::sem {
    ...
    namespace {
        ...
        DefinitionProcessingResult processSimpleDefinition (
            ...
            return [&] (CompleteValue && definition) -> DefinitionProcessingResult {
                ...
                auto translationResult = interface::translateValue(ctx) || target::category{} <<= std::move(definition);
                if (!translationResult) return translationResult.error();
                auto translated = *std::move(translationResult);

                return runtimeSimpleDefinition(ctx, type, type.translateType(), translated.expression);
    ...
    DefinitionProcessingResult type::In::processDefinition (
        ...
        if (!definition) {
            ...
            auto translationResult = interface::translateType || target::category{} <<= valueType;
            if (!translationResult) return translationResult.error();
            auto typeName = *std::move(translationResult);
    ...
    */
    ...
    DefinitionProcessingResult type::Array::processDefinition (
        ...
        auto translationResult = interface::translateTypeSpecifier || target::category{} <<= *type;
        if (!translationResult) return translationResult.error();
        auto valueType = *std::move(translationResult);
    ...
    namespace {
        ...
        std::string bufferTypeName (Integral size) {
