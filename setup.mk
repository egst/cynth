...
TOS       = $(subst LINUX,linux,$(subst WINDOWS,win,$(PLATFORM)))
TAPI      = $(subst STDOUT,stdout,$(subst ASIO,asio,$(DRIVERAPI)))
TOUT      = $(subst DEBUG,debug,$(subst STAT,stat,$(SYNTHOUT)))
TARGET    = $(TOS)-$(TAPI)-$(TOUT)
TARGETDIR = $(TARGET)/
...
ASIO_OBJ := $(ASIO_SRC:%.cpp=$(BIN_EXTERN)$(TARGETDIR)%$(EXT_OBJ))
...
# Note: Files conforming to the basic structure
# (inc/, src/ and entry/ as specified in config.mk)
# are handled automatically.
# Other files must be specified here.

CONTROL_ENTRY_DEP = $(DEP_ENTRY)control$(EXT_DEP)
CONTROL_ENTRY_OBJ = $(BIN_ENTRY)$(TARGETDIR)control$(EXT_OBJ)
COMPILE_COMMANDS  = compile_commands.json

DEP_SRC_FILES   +=
DEP_ENTRY_FILES += $(CONTROL_ENTRY_DEP)
OTHER_FILES     += $(CONTROL_ENTRY_OBJ)
OTHER_FILES     += $(COMPILE_COMMANDS)
...
LINK_OPTIONS       +=
SYNTH_COMP_OPTIONS += -std=$(SYNTH_STD)
