...
ADDITIONAL_OPTIONS = -static -Wall -Wextra -pedantic -fdiagnostics-color=always

# Additional linkage (with the chosen compiler), dependency generation (with GCC), Lex and Bison options:
LINK_OPTIONS  = -fdiagnostics-color=always
DEP_OPTIONS   = -fdiagnostics-color=always
FLEX_OPTIONS  =
...
BIN_DIST_FILES  = $(ENTRY_FILES:$(ENTHY)%$(EXT_IMPL)=$(BIN_DIST)%$(EXT_EXE))
...
# Generate parser:
GEN_PARSER = $(BISON) $(BISON_OPTIONS) -d -o $(SRC)$(IMPL_PARSER)$(EXT_IMPL) $(GEN)parser.y

#### TARGETS ####

.PHONY: all gen dep obj exe clean clean-dist clean-bin clean-dep clean-gen
...
$(SRC)$(IMPL_PARSER)$(EXT_IMPL) $(SRC)$(INC_PARSER)$(EXT_HEAD): $(GEN)parser.y
